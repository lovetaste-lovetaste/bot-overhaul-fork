...
public void OnPluginStart()
{
	...
	g_hCVPub = CreateConVar("sm_gbmw_pubspy", "1", "If nonzero, force spies to use the Pub Spy Set.", FCVAR_NONE, true, 0.0, true, 1.0);
...
public void OnMapStart()
{
	...
	g_hCVMelee = FindConVar("tf_bot_melee_only");
	g_diff = FindConVar("tf_bot_difficulty");
}
...
public Action Timer_GiveWeapons(Handle timer, any data)
{
	...
	case TFClass_Pyro:
		{
			...
			int rnd3 = GetRandomUInt(1,9);
			int axePyro = GetRandomUInt(1,2);
			TF2_RemoveWeaponSlot(client, 2);
			
			
			if(axePyro == 1)
			{
				PrintToConsoleAll("Axe Pyro has been spawned.");
				int rndAxe = GetRandomUInt(1,4);
					switch (rndAxe)
					{
					case 1:
						{
							CreateWeapon(client, "tf_weapon_fireaxe", 38, 6, 1, 1);
						}
					case 2:
						{
							CreateWeapon(client, "tf_weapon_fireaxe", 457, 6, 2, 1);
						}
					case 3:
						{
							CreateWeapon(client, "tf_weapon_fireaxe", 1000, 6, 3, 1);
						}
					case 4:
						{
							CreateWeapon(client, "tf_weapon_fireaxe", 38, 9, 4, 1);
						}
					}
			}
			{
	...
	case TFClass_Spy:
		{
		int pubspyrnd = GetRandomUInt(0,1);
		//int pubspyrnd = 1;
		if(pubspyrnd == 1 || g_hCVMelee != 0 || g_bMedieval || g_hCVPub != 0) //This spawns a Letranger/Amby - Kunai - Dead Ringer spy. I made it always spawn this in melee only servers due to it being basically the best loadout. This is ESPECIALLY true if the bot is against easy bots, because it can get huge chainstabs.
			{
				//PrintToChatAll("Pub Spy has been spawned.");
				if (!g_bMedieval)
				{
					int gunPubRnd = GetRandomUInt(0,2);
					TF2_RemoveWeaponSlot(client, 0);
					switch (gunPubRnd)
					{
						case 1:
						{
							CreateWeapon(client, "tf_weapon_revolver", 61, 11, 2, 1);
						}
						case 2:
						{
							CreateWeapon(client, "tf_weapon_revolver", 224, 11, 3, 1);
						}
						case 3:
						{
						CreateWeapon(client, "tf_weapon_revolver", 1006, 11, 1, 1);
						}
					}
				}
					TF2_RemoveWeaponSlot(client, 2);
					CreateWeapon(client, "tf_weapon_knife", 356, 11, 0, 1);
					TF2_RemoveWeaponSlot(client, 4);
					CreateWeapon(client, "tf_weapon_invis", 59, 11, 5, 1);
			}
		else
		{
			if (!g_bMedieval)
...
public int StrangifyWeapon(int difficulty=-1)
{
	if(difficulty >= 0 && difficulty <= 3)
	{
		if(difficulty == 0)
		{
			TF2Attrib_SetByDefIndex(weapon, 214, view_as<float>(GetRandomInt(0, 2000)));
		}
		if(difficulty == 1)
		{
			TF2Attrib_SetByDefIndex(weapon, 214, view_as<float>(GetRandomInt(2000, 6000)));
		}
		if(difficulty == 2)
		{
			TF2Attrib_SetByDefIndex(weapon, 214, view_as<float>(GetRandomInt(6000, 8000)));
		}
		if(difficulty == 3)
		{
		TF2Attrib_SetByDefIndex(weapon, 214, view_as<float>(GetRandomInt(8000, 18000)));
		}
	}
	else
	{
...
bool CreateWeapon(int client, char[] classname, int itemindex, int quality = 6, int level = 0, int ks = 0)
{
	...
	if(ks)
	{
		//this is specifically for the pub spies to always have a strange pro ks effect
		SetEntData(weapon, FindSendPropInfo(entclass, "m_iEntityQuality"), 11);
		StrangifyWeapon(g_diff);
		TF2Attrib_SetByDefIndex(weapon, 2025, 3.0);
		TF2Attrib_SetByDefIndex(weapon, 2014, GetRandomInt(1,7) + 0.0);
