				...
				*/
				...
				if(g_Spy.IntValue == 1)
				{
					...
					{	
						...
						//the searchOriginView and searchOriginVelView are QoL changes for spectators of Spy bots. They raise the Y value so that the bots target near the center of the back, instead of the bottom of the model. Primarily done to not only make the bot more realistic but to also make it better to actually spec the bot. Before I did this it would just stare at its origin, and since the origin is at the bottom of the model, it would always look weird
					...
					//	PrintToConsoleAll("angles", chainAngles);
						...
						// this was here due to showins bot overhaul. However, due to my changes this shouldnt be needed. I may use it in the future, but as for now, its fine to leave this out
						...
						//IsPointVisible(clientOrigin, searchOrigin)
						 //&& chainDistance < 1000
						 // && 
						if (IsTargetInSightRange(client, Ent)) //otherclass != TFClass_Pyro (disabled so the spy would actually try and kill enemy Pyros instead of being a pussy).
						{
								...
								else
								{
									if(IsValidEntity(knife) && ((GetHealth(client) > 45 && otherclass == TFClass_Spy) || (GetHealth(client) > 65 && otherclass != TFClass_Spy) || 356 == GetEntProp(knife, Prop_Send, "m_iItemDefinitionIndex") || 461 == GetEntProp(knife, Prop_Send, "m_iItemDefinitionIndex")))// IF the bot has a knife AND it is either above 45hp against a Spy, above 65hp against a non-Spy, has a kunai, or has a big earner, THEN it will proceed.
									{
										...
										if(chainDistance > 50.0) 
										{
											...
											}	//it changes the spy shuffle direction when the current X velocity reaches 319, or with a 20% chance to ignore that and switch anyway. The random chance is added so the spy doesn't move perfectly, allowing for more confusion on the enemy and more tricking. Kinda doesnt work but ill fix l8r
											if(chainmode)
											{
												moveSide(vel, 400);
											}
